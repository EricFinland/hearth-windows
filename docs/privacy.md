...
| the update feed in `release/trust.json` | an update check | a request for a signed release manifest. **This is currently pointed at a reserved name that can never resolve**, so no install can reach an update host at all. See [updates.md](updates.md) |
